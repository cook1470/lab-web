...
const LAYERS = [
  ...
  { min: 20, name: '根系', dark: '#4e3622', light: '#6a4a2e', deco: '#b8905a', val: 7, hard: 2 },
  { min: 70, name: '濕土', dark: '#3e2c1e', light: '#56402c', deco: '#8aa05a', val: 12, hard: 4 },
  { min: 180, name: '碎石層', dark: '#4a4238', light: '#625a4e', deco: '#c0b49a', val: 20, hard: 8 },
  { min: 350, name: '黏土', dark: '#6b3a2c', light: '#8a5240', deco: '#d98a5a', val: 34, hard: 14 },
  { min: 700, name: '地下水', dark: '#1e2e2e', light: '#2e4444', deco: '#6ec8c0', val: 56, hard: 22 },
  { min: 1400, name: '古老土層', dark: '#241a08', light: '#3a2c10', deco: '#ffd76a', val: 90, hard: 34 },
];
...
const ORECOL = { 10: ['#b08a4a', '#e6c88a'], 11: ['#d9a55a', '#f6dca4'], 12: ['#f2a830', '#ffe08a'], 13: ['#3a6a4a', '#7ac08a'],
  14: ['#c0304a', '#ff8aa0'], 15: ['#e8e8f0', '#ffffff'], 20: ['#e8dcc0', '#fff8e4'], 21: ['#c87a9a', '#f4c8dc'], 22: ['#e08a1e', '#ffd070'], 23: ['#fff2a0', '#ffffff'] };
...
function drawSky(offX, sx, sy) {
  ...
  const cx = WW * T / 2;
  g.fillStyle = '#7a5a3a'; for (let r = 0; r < 26; r++) { const w = Math.round(Math.sqrt(26 * 26 - r * r) * 1.6); g.fillRect(cx - w, -r - 1, w * 2, 1); }
  g.fillStyle = '#9a7650'; for (let i = 0; i < 40; i++) { const a = hash(i, 7) * Math.PI, rr = hash(i, 9) * 24; g.fillRect(Math.round(cx + Math.cos(a) * rr * 1.6), Math.round(-Math.sin(a) * rr) - 1, 1, 1); }
  g.fillStyle = '#1a120c'; g.fillRect(cx - 4, -8, 8, 8); g.fillRect(cx - 3, -10, 6, 2);
  g.fillStyle = '#6aa83c'; for (let i = 0; i < WW * T; i += 7) g.fillRect(i, -4 - (i * 13 % 3), 1, 3);
...
function drawMinerSprite(u, now) {
  ...
  const face = u.mode === 'return' ? -1 : 1; // 回程頭朝上
  const leg = (Math.floor(now / 90) + u.id) % 2;
  const body = '#d0502a', hi = '#ff9a5a';
  // 通道是深色的：先描一圈亮邊再畫身體，不然螞蟻會融進坑道裡
  g.fillStyle = '#f4e2c0'; g.fillRect(bx2 + 0, by2 + 3, 10, 5); g.fillRect(bx2 + 9, by2 + 1, 2, 3);
  // 三節身體（橫向：頭在右）
  g.fillStyle = body; g.fillRect(bx2 + 1, by2 + 4, 3, 3); g.fillRect(bx2 + 4, by2 + 5, 2, 2); g.fillRect(bx2 + 6, by2 + 4, 3, 3);
  g.fillStyle = hi; g.fillRect(bx2 + 2, by2 + 4, 1, 1); g.fillRect(bx2 + 7, by2 + 4, 1, 1);
  // 腳（會動）
  g.fillStyle = body;
  for (let k = 0; k < 3; k++) { const lx = bx2 + 2 + k * 2, off = (k + leg) % 2; g.fillRect(lx - off, by2 + 7, 1, 2); }
  // 觸角
  g.fillRect(bx2 + 9, by2 + 3, 1, 1); g.fillRect(bx2 + 10, by2 + 2, 1, 1);
  // 大顎：在挖的時候開合
  if (u.dig && leg) { g.fillStyle = '#8a5a3a'; g.fillRect(bx2 + 9, by2 + 5, 2, 1); }
  // 搬東西：背上頂著一顆食物
  if (u.cargo.length) { const c = ORECOL[u.cargo[u.cargo.length - 1]] || ['#e6c88a', '#fff']; g.fillStyle = c[0]; g.fillRect(bx2 + 3, by2 + 1, 3, 3); g.fillStyle = c[1]; g.fillRect(bx2 + 3, by2 + 1, 1, 1); }
}
...
function icon(kind, x, y) {
  ...
  if (kind === 'drill') { g.fillStyle = '#c08a5a'; g.fillRect(x, y, 2, 5); g.fillRect(x + 5, y, 2, 5); g.fillRect(x + 1, y + 5, 2, 2); g.fillRect(x + 4, y + 5, 2, 2); }
  if (kind === 'tank') { g.fillStyle = '#6ec8ff'; g.fillRect(x + 3, y, 1, 2); g.fillRect(x + 2, y + 2, 3, 2); g.fillRect(x + 1, y + 4, 5, 3); g.fillStyle = '#e8ffff'; g.fillRect(x + 2, y + 4, 1, 1); }
  if (kind === 'bag') { g.fillStyle = '#6aa83c'; g.fillRect(x + 1, y + 1, 5, 4); g.fillRect(x + 2, y, 3, 1); g.fillRect(x + 2, y + 5, 3, 1); g.fillStyle = '#3a6a20'; g.fillRect(x + 3, y + 1, 1, 5); }
  if (kind === 'down') { g.fillStyle = '#9ee7ff'; g.fillRect(x + 2, y, 2, 4); g.fillRect(x, y + 3, 6, 1); g.fillRect(x + 1, y + 4, 4, 1); g.fillRect(x + 2, y + 5, 2, 1); }
  if (kind === 'miner') { g.fillStyle = '#e8d8c0'; g.fillRect(x, y + 3, 2, 2); g.fillRect(x + 2, y + 4, 1, 1); g.fillRect(x + 3, y + 3, 3, 2); g.fillRect(x + 6, y + 1, 1, 2); g.fillRect(x, y + 5, 1, 2); g.fillRect(x + 2, y + 5, 1, 2); g.fillRect(x + 4, y + 5, 1, 2); }
...
function drawHud(now) {
  ...
  icon('down', 6, 16); const dm = '' + Math.floor(state.depth); txt(dm, 16, 17, '#9ee7ff');
