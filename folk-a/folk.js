...
'use strict';
(function () {
  // 玩家層掛勾（各版本的 layer.js 先定義 window.FOLK_LAYER）
  const HK = window.FOLK_LAYER || {};
  ...
  function decide(p) {
    ...
    if (HK.decide && HK.decide(p)) return;
    if (rain > 0 && !(p.diligent || p.assign)) { goTo(p, p.bed[0] + rr(-3, 3), p.bed[1] + 6); return setState(p, 'shelter', 4); }
  ...
  function step(dt) {
    T += dt;
    if (HK.step) HK.step(dt);
    if (rain > 0) { rain -= dt; if (rain <= 0) { log([], '雨停了'); nextRain = T + rr(60, 100); } }
    else if (T >= nextRain && !(HK.blockRain && HK.blockRain())) { rain = rr(20, 28); log([], '開始下雨'); }
    for (const c of crops) if (c.g < 1) c.g = Math.min(1, c.g + dt * (rain > 0 ? 0.03 : 0.011) * (HK.growMul ? HK.growMul() : 1));
    ...
    for (const p of folk) {
      ...
      // 淋雨生病
      if (rain > 0 && !inHouse(p) && !p.down && !(HK.sheltered && HK.sheltered(p))) {
        p.wet += dt;
        if (p.wet > 9 && p.sick <= 0 && rnd() < 0.05 * dt) {
          p.sick = rr(35, 45); release(p); dropCarry(p);
          log([p], N(p) + '淋雨太久，生病了，回床上躺'); if (HK.accident) HK.accident('sick', p);
      ...
      if (p.hunger <= 0 && p.state !== 'sickBed' && p.state !== 'pinned') {
        ...
        if (p.starve > 25 && p.state !== 'starving') {
          ...
          log([p], N(p) + '餓到倒在地上起不來'); if (HK.accident) HK.accident('starving', p);
        }
      }
      if (p.state === 'starving') {
  ...
  function tick(p, dt) {
    ...
    switch (p.state) {
      ...
      case 'toCrop':
        if (rain > 0 && !(p.diligent || p.assign) && !p.eatOnField) { release(p); decide(p); break; }
      ...
      case 'deliver':
        ...
        break;
      case 'toTree':
        if (rain > 0 && !(p.diligent || p.assign)) { release(p); decide(p); break; }
      ...
      case 'falling':
        if (p.timer <= 0) {
          ...
          log([p], N(p) + '砍倒的樹壓在自己身上，動不了'); if (HK.accident) HK.accident('pinned', p);
          // 附近的人嚇到，跑過來圍著看
          for (const o of folk) {
  ...
  function drawWorld() {
    ...
    if (rain <= 0 || (HK.fireLit && HK.fireLit())) { const fl = Math.floor(T * 6) % 2; R(FIRE.x - 1, FIRE.y - 2 + fl, 3, 3, '#ef7d57'); R(FIRE.x, FIRE.y - 3 + fl, 1, 2, '#ffcd75'); }
    // 田
  ...
  if (HK.init) HK.init({
    folk, crops, TREES, FIRE, FIELD, HOUSE, SHED, WELL, BEDS, W, H, C, PLAIN, cv,
    get T() { return T; }, get food() { return food; }, set food(v) { food = v; },
    get rain() { return rain; }, set rain(v) { rain = v; }, get nextRain() { return nextRain; }, set nextRain(v) { nextRain = v; },
    log, N, decide, goTo, setState, shedSpot, release, dropCarry, eat, rr, rnd, pick, arrived, dist, inHouse,
  });
  requestAnimationFrame(frame);
})();
